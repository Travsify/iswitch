...
class ChatbotController extends Controller
{
    ...
    private function handleFlightSearch(string $msg): array
    {
        ...
        if (preg_match('/from\s+(\w[\w\s]*?)\s+to\s+(\w[\w\s]*)/i', $msg, $matches)) {
            $origin = trim($matches[1]); $destination = trim($matches[2]);
        } elseif (preg_match('/(?:flights?|fly|going)\s+to\s+(\w[\w\s]*)/i', $msg, $matches)) {
            $destination = trim($matches[1]);
        }
        $query = Flight::query();
        if ($origin) $query->where('departure_airport', 'like', '%' . $origin . '%');
        if ($destination) $query->where('arrival_airport', 'like', '%' . $destination . '%');
    ...
    private function handleHotelSearch(string $msg): array
    {
        ...
        $query = Hotel::query();
        if ($location) $query->where('location', 'like', '%' . $location . '%')->orWhere('name', 'like', '%' . $location . '%');
    ...
    private function handleVisaCheck(string $msg): array
    {
        ...
        foreach ($visaData as $key => $data) {
            if (str_contains($countryLower, $key)) {
                $found = $data; $country = ucfirst($key); break;
            }
        }
        if ($found) {
    ...
    private function handleInsurance(string $msg): array
    {
        ...
        $plans = [
            ['name' => 'Basic Travel Guard', 'price' => '$25', 'coverage' => '$50,000'],
            ['name' => 'Premium Shield', 'price' => '$55', 'coverage' => '$250,000'],
            ['name' => 'Platinum Cover', 'price' => '$95', 'coverage' => '$1,000,000'],
        ];
    ...
    {
        return ['reply' => $text, 'type' => $type, 'data' => $data, 'timestamp' => now()->toIso8601String()];
    }
}
